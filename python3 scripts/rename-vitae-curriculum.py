...
def slugify(text: str) -> str:
    ...
    strips bracket prefixes like "[01] -", and keeps a .md suffix if present.
    """
    t = text.strip()
...
def canonical_file_name(name: str) -> str:
    """File naming rules (strip grade prefix, strip 'The X -' prefix, slugify)."""
    raw = name.strip()

    # Fix trailing spaces before extension: "Kernel Set .md"
...
def build_plan(root: Path) -> List[Move]:
    """
    Build a bottom-up rename plan for everything under `root`.

    - Directories: renamed via canonical_dir_name()
    - Files: only .md files renamed via canonical_file_name()
    - Bottom-up ordering avoids breaking paths during parent folder renames.
    """
    all_paths: List[Path] = []

    for p in root.rglob("*"):
        # skip any embedded tree dumps you placed under curriculum
...
def apply_moves(moves: List[Move], repo_root: Path, apply: bool) -> int:
    ...
    for m in moves:
        ...
        if apply:
            if use_git:
                ...
                if code != 0:
                    print(out.rstrip())
                    raise SystemExit(f"git mv failed: {m.src} -> {m.dst}")
            else:
                m.src.rename(m.dst)
...
def main() -> None:
    ...
    changed = apply_moves(moves, repo_root=repo_root, apply=args.apply)
