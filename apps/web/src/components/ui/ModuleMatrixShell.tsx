...
export default function ModuleMatrixShell({
  ...
  const headerActions = (
    <div className="flex items-start justify-end" data-no-route-swipe="true">
      <nav aria-label="Horizontal card navigation" className="relative h-[1.78rem] shrink-0" style={{ width: `${railWidth + 2}px` }}>
        <span
          aria-hidden="true"
        ...
        {horizontalTabs.map((tab, index) => {
          ...
          return (
            <span
              ...
              aria-hidden="true"
              className="pointer-events-none absolute z-30 h-[14px] w-px bg-white/12"
          ...
          style={{
            ...
            borderRadius:
              activeHorizontalIndex === 0
                ? '0.86rem 0.38rem 0.38rem 0.86rem'
                : activeHorizontalIndex === segmentCount - 1
                  ? '0.38rem 0.86rem 0.86rem 0.38rem'
                  : '0.38rem',
