...
export function NexusModuleScreen({ family }: { family: NexusFamilyId }) {
  ...
  const onSaveProposalDraft = async () => {
    setMessage(null);
    if (!acc?.trusted) return setMessage('Activate your ACC to save a Nexus draft.');
    if (!body.trim()) return setMessage('Write something first.');
    setBusy(true);
    try {
  ...
  const families = useMemo<Record<NexusFamilyId, FamilyConfig>>(
    () => ({
      current: {
        href: ORDER[0],
        label: 'Current',
        shellAction: <div className="text-xs text-zinc-400">A track · stream, filters, and attachments</div>,
        cards: [
          {
            id: 'a1',
            navLabel: 'A1',
            title: 'Nexus - A1 Stream',
            caption: 'The living current. This is where the shared stream flows in time order and where the user returns to see what is happening now across the network.',
            render: () => <FeedCard rows={rows} loading={loadingFeed} previews={previews} />,
          },
          {
            id: 'a2',
            navLabel: 'A2',
            title: 'Nexus - A2 Filters',
            caption: 'The stream should become navigable. Saved filters, topical slices, celestial filters, and personal views belong here so the current can be shaped without breaking time order.',
            render: () => <FiltersCard rows={rows} nexusState={nexusState} />,
          },
          {
            id: 'a3',
            navLabel: 'A3',
            title: 'Nexus - A3 Media',
            caption: 'The attachment layer. Images, video, audio, and later documents give the stream texture and make Nexus feel like a living media network instead of a text-only feed.',
            render: () => <MediaCard rows={rows} previews={previews} />,
          },
        ],
      },
      post: {
        href: ORDER[1],
        label: 'Post',
        shellAction: <div className="text-xs text-zinc-400">B track · compose, draft, and publish your trace</div>,
        cards: [
          {
            id: 'b1',
            navLabel: 'B1',
            title: 'Nexus - B1 Compose',
            caption: 'The creation surface. This is where the user writes, attaches media, chooses a destination, and prepares a post before it enters the stream.',
            render: () => <ComposeCard target={target} setTarget={setTarget} roomId={roomId} body={body} setBody={setBody} media={media} onPickFiles={onPickFiles} />,
          },
          {
            id: 'b2',
            navLabel: 'B2',
            title: 'Nexus - B2 Drafts',
            caption: 'The local memory shelf. Drafts and proposals belong here before publication so the user can shape intention without immediately entering the public stream.',
            render: () => <PostDraftCard acc={acc} proposalTitle={proposalTitle} setProposalTitle={setProposalTitle} proposalKind={proposalKind} setProposalKind={setProposalKind} includeTempusContext={includeTempusContext} setIncludeTempusContext={setIncludeTempusContext} onSaveProposalDraft={onSaveProposalDraft} busy={busy} body={body} nexusContext={nexusContext} nexusState={nexusState} />,
          },
          {
            id: 'b3',
            navLabel: 'B3',
            title: 'Nexus - B3 Publish',
            caption: 'The release surface. Helia-backed content enters a Matrix room here, with posting cost, publication status, and network trace kept visible.',
            render: () => <PublishCard acc={acc} body={body} media={media} busy={busy} message={message} onPublish={onPublish} />,
          },
        ],
      },
      channel: {
        href: ORDER[2],
        label: 'Channels',
        shellAction: <div className="text-xs text-zinc-400">C track · directory, room creation, and access economics</div>,
        cards: [
          {
            id: 'c1',
            navLabel: 'C1',
            title: 'Nexus - C1 Directory',
            caption: 'The room map. Public spaces, their names, and their membership counts live here so the user can discover the bounded worlds of Nexus.',
            render: () => <DirectoryCard rooms={rooms} loading={loadingRooms} />,
          },
          {
            id: 'c2',
            navLabel: 'C2',
            title: 'Nexus - C2 Create',
            caption: 'The world-building surface. Trusted users can open a new room here, set entry economics, and establish the shell of a new community space.',
            render: () => <CreateChannelCard acc={acc} channelName={channelName} setChannelName={setChannelName} channelPrice={channelPrice} setChannelPrice={setChannelPrice} onCreateChannel={onCreateChannel} />,
          },
          {
            id: 'c3',
            navLabel: 'C3',
            title: 'Nexus - C3 Access',
            caption: 'The fixed room rules. Creation cost, upkeep, entry economics, and the boundary between paid access and real authority remain explicit here.',
            render: () => <AccessCard message={message} />,
          },
        ],
      },
    }),
      ...
      <div className="h-full min-h-0" onTouchStartCapture={onTouchStartCapture} onTouchMoveCapture={onTouchMoveCapture} onTouchEndCapture={onTouchEndCapture}>
        <ModuleMatrixShell
          title={
            ...
            >
              <span className="tempus-title-word truncate">Nexus</span>
...
function FeedCard({ rows, loading, previews }: { rows: Row[]; loading: boolean; previews: Record<string, MediaView> }) {
  return (
    <div className="space-y-3">
      {loading ? (
        <div className="text-sm text-zinc-400">Loading stream…</div>
      ) : rows.length === 0 ? (
        <div className="text-sm text-zinc-400">No current messages found.</div>
      ) : (
        rows.slice(0, 8).map((r, i) => (
          <article key={i} className="rounded-2xl border border-white/10 bg-black/40 p-4">
            <div className="text-xs text-zinc-400">{r.author || 'unknown'} {r.when ? `· ${new Date(r.when).toLocaleString()}` : null}</div>
            {r.body ? <div className="mt-1 break-words whitespace-pre-wrap text-sm">{r.body}</div> : null}
            {r.media && r.media.length > 0 ? (
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                {r.media.map((m, idx) => (
                  <div key={idx} className="rounded-xl border border-white/10 bg-black/30 p-2">
                    {renderMedia(m, previews)}
                    <div className="mt-1 truncate text-[11px] text-zinc-500">{m.name || m.cid}</div>
                  </div>
                ))}
              </div>
            ) : null}
...
function FiltersCard({ rows, nexusState }: { rows: Row[]; nexusState: NexusState }) {
  const mediaCount = rows.reduce((count, row) => count + (row.media?.length ?? 0), 0);
  return (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,.95fr)]">
      <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-4">
        <div className="text-[11px] uppercase tracking-[0.18em] text-zinc-500">Filter scaffold</div>
        <h3 className="mt-2 text-base font-semibold text-zinc-100">How the current will become navigable</h3>
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <StateChip title="Temporal" detail="Newest first, hourly boosts, and present-window relevance." />
          <StateChip title="Celestial" detail="Later filters for signs, planets, phases, or active layers." />
          <StateChip title="Topic" detail="Keyword and theme filters for focused reading." />
          <StateChip title="Personal" detail="Saved presets and followed channels or creators." />
        </div>
      </div>
      <div className="grid gap-3">
        <StateChip title="Visible posts" detail={`${rows.length} items in current slice`} />
        <StateChip title="Media attachments" detail={`${mediaCount} attachments in current slice`} />
        <StateChip title="Local drafts" detail={`${nexusState.proposals.length} saved drafts`} />
      </div>
    </div>
  );
}
...
function MediaCard({ rows, previews }: { rows: Row[]; previews: Record<string, MediaView> }) {
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-black/20 p-4">
      <div className="text-[11px] uppercase tracking-[0.18em] text-zinc-500">Media surface</div>
      <h3 className="mt-2 text-base font-semibold text-zinc-100">Helia-backed attachments</h3>
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        {media.length === 0 ? (
          <div className="text-sm text-zinc-400">No media attachments found in the current slice.</div>
        ) : (
          media.slice(0, 8).map((m, idx) => (
            <div key={idx} className="rounded-2xl border border-white/10 bg-black/30 p-3">
              {renderMedia(m, previews)}
              <div className="mt-2 truncate text-xs text-zinc-500">{m.name || m.cid}</div>
            </div>
          ))
        )}
      </div>
  ...
  return (
    <div className="space-y-4 rounded-3xl border border-white/10 bg-white/[0.04] p-4">
      <div className="grid gap-2 sm:grid-cols-2">
        <input value={target} onChange={(e) => setTarget(e.target.value)} placeholder="Room alias or ID" className="w-full rounded-2xl border border-white/10 bg-black/20 p-3 text-sm outline-none focus:border-amber-300/50" />
        <div className="flex items-center text-xs text-zinc-400">Resolved roomId:<span className="ml-1 truncate">{roomId || '—'}</span></div>
      </div>
      <textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="Write something cosmic…" className="w-full resize-none rounded-2xl border border-white/10 bg-black/20 p-3 text-sm outline-none focus:border-amber-300/50" rows={5} />
      <div className="space-y-2">
        <label className="inline-block">
          <span className="sr-only">Attach files</span>
          <input type="file" multiple onChange={(e) => onPickFiles(e.target.files)} className="block text-xs" accept="image/*,video/*,audio/*,application/pdf" />
        </label>
        {media.length > 0 ? (
          <ul className="space-y-1">
            {media.map((m, i) => (
              <li key={i} className="flex items-center gap-2 text-xs text-zinc-300">
                ...
                {m.cid ? <span className="truncate opacity-70">CID: {m.cid}</span> : null}
              </li>
            ))}
          </ul>
        ) : null}
      ...
      <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-4">
        <div className="grid gap-2 sm:grid-cols-2">
          ...
          <select value={proposalKind} onChange={(e) => setProposalKind(e.target.value)} className="w-full rounded-2xl border border-white/10 bg-black/20 p-3 text-sm outline-none focus:border-amber-300/50">
            ...
            <option value="channel">Channel draft</option>
          </select>
        </div>
        <label className="mt-4 flex items-start gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-3 text-xs text-zinc-400">
          <input type="checkbox" checked={includeTempusContext} onChange={(event) => setIncludeTempusContext(event.target.checked)} className="mt-1" />
          <span>
            <span className="block text-sm text-zinc-100">Attach Tempus context</span>
            <span>Factual timing context only. It does not schedule or authorize the draft.</span>
          </span>
        </label>
        <div className="mt-4 flex justify-end">
          <button onClick={onSaveProposalDraft} disabled={busy || !body.trim()} className="rounded-md border border-white/20 px-3 py-1.5 text-xs text-zinc-200 hover:bg-white/10 disabled:opacity-60">{busy ? 'Saving…' : 'Save draft'}</button>
        </div>
      </div>
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,.95fr)]">
        <div className="rounded-3xl border border-white/10 bg-black/20 p-4 text-sm text-zinc-300">
          <div className="text-[11px] uppercase tracking-[0.18em] text-zinc-500">Draft context</div>
          <div className="mt-2">{nexusContext.version} · {nexusContext.visibility} · {nexusContext.source}</div>
          {nexusContext.warnings.length > 0 ? <ul className="mt-3 list-disc space-y-1 pl-4 text-xs text-zinc-400">{nexusContext.warnings.map((warning: string) => <li key={warning}>{warning}</li>)}</ul> : null}
        </div>
        <div className="rounded-3xl border border-white/10 bg-black/20 p-4 text-sm text-zinc-300">
          <div className="text-[11px] uppercase tracking-[0.18em] text-zinc-500">Saved drafts</div>
          <div className="mt-2">{nexusState.proposals.length} local draft{nexusState.proposals.length === 1 ? '' : 's'} saved.</div>
        </div>
      </div>
    </div>
  );
      ...
      {!acc?.trusted ? (
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-[11px] uppercase tracking-[0.18em] text-zinc-500">Publish</div>
              <h3 className="mt-2 text-base font-semibold text-zinc-100">Helia + Matrix release</h3>
            </div>
            <div className="text-xs text-zinc-400">Cost: {COST.POST} MANA</div>
          </div>
          <div className="mt-4 flex justify-end">
            <button onClick={onPublish} disabled={busy || (!body.trim() && media.length === 0)} className="rounded-md border border-amber-300/40 px-3 py-1.5 text-xs text-amber-300 hover:bg-amber-300/10 disabled:opacity-60">{busy ? 'Publishing…' : 'Publish'}</button>
          </div>
          {message ? <div className="mt-3 text-xs text-amber-300">{message}</div> : null}
        </div>
      )}
      <div className="rounded-3xl border border-white/10 bg-black/20 p-4 text-sm text-zinc-300">
        Posts are Helia-backed and shared into Matrix rooms. Publication affects visibility, not governance authority or human worth.
      </div>
    </div>
  );
}
...
function DirectoryCard({ rooms, loading }: { rooms: any[]; loading: boolean }) {
  return (
    <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-4">
      ...
      <h3 className="mt-2 text-base font-semibold text-zinc-100">Channel directory</h3>
      <div className="mt-4 space-y-2">
        {loading ? (
          <div className="text-sm text-zinc-400">Loading channels…</div>
        ) : (
          rooms.map((r) => (
            <div key={r.room_id} className="rounded-2xl border border-white/10 bg-black/30 p-3">
...
function CreateChannelCard({ acc, channelName, setChannelName, channelPrice, setChannelPrice, onCreateChannel }: any) {
  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-4 space-y-3">
      <input value={channelName} onChange={(e) => setChannelName(e.target.value)} placeholder="Channel name" className="w-full rounded-2xl border border-white/10 bg-black/20 p-3 text-sm outline-none focus:border-amber-300/50" />
      <input value={channelPrice} onChange={(e) => setChannelPrice(Number(e.target.value) || 0)} type="number" min={0} step={1} placeholder="Join price (MANA, optional)" className="w-full rounded-2xl border border-white/10 bg-black/20 p-3 text-sm outline-none focus:border-amber-300/50" />
      <button onClick={onCreateChannel} className="rounded-md border border-amber-300/40 px-3 py-1.5 text-xs text-amber-300 hover:bg-amber-300/10">Create channel (−{COST.CREATE_CHANNEL} MANA)</button>
    </div>
  );
}
...
function AccessCard({ message }: { message: string | null }) {
  return (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
      <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-4">
        <div className="text-[11px] uppercase tracking-[0.18em] text-zinc-500">Access economics</div>
        <h3 className="mt-2 text-base font-semibold text-zinc-100">Creation and upkeep</h3>
        <div className="mt-4 space-y-3">
          <PolicyLine text={`Creating costs ${COST.CREATE_CHANNEL} MANA.`} />
          <PolicyLine text={`Upkeep is ${COST.MAINTAIN_CHANNEL} MANA/month.`} />
          <PolicyLine text="Channel cost affects access, not governance authority." />
        </div>
      </div>
      <div className="rounded-3xl border border-white/10 bg-black/20 p-4">
        <div className="text-[11px] uppercase tracking-[0.18em] text-zinc-500">Latest status</div>
        <h3 className="mt-2 text-base font-semibold text-zinc-100">Room message</h3>
        <div className="mt-4 text-sm text-zinc-300">{message ?? 'No channel action message yet.'}</div>
      </div>
    </div>
  );
}

function StateChip({ title, detail }: { title: string; detail: string }) {
  return (
...
function PolicyLine({ text }: { text: string }) {
  return <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3 text-sm text-zinc-300">{text}</div>;
}
