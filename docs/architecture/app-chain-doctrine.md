...
description: "Boundary contract between the App surface, ARCnet settlement layer, and Doctrine/Governance constraints."
...
\- \*\*ARCnet\*\* — the settlement surface (slow, public, minimal, factual)

\- \*\*Doctrine\*\* — the constraint surface (canonical invariants; what must never drift)



If any one layer begins to behave like another, the system becomes unstable:
...
\### 1.2 ARCnet layer (settlement + receipts)
...
\- Receipts must never state \*interpretations\* (“X advanced,” “X is better,” “X is worthy”).
...
\- emit private content into events
...
The App may render receipts meaningfully, but must not claim the chain \*means\* something it doesn’t.
...
\- content bodies live off-chain

\- chain stores hashes/pointers + receipts where required
...
\- logs should support reliability, not surveillance
...
\## 5) Doctrine gates (how doctrine constrains code)
...
\- CI checks verify canonical file paths and naming

\- checksum integrity for constitutional artifacts (when enabled)

\- no “forbidden terms” in on-chain event schemas (score/rank/virtue)
...
\- upgrades must be transparent and auditable
...
\- Does this feature require settlement? If no, keep it off-chain.

\- Does this event encode meaning? If yes, redesign it.

\- Does this UX create pressure loops? If yes, remove/soften until silence is valid.

\- Can payment accelerate progression? If yes, it violates doctrine.

\- Can we audit truth from receipts without storing content? If no, receipts are poorly designed.

\- Can a user participate without being tracked? If no, privacy posture is broken.
...
\- System overview: `../architecture/arcanum-system-overview.md`

\- Canonical modules: `../architecture/canonical-modules.md`

\- Layer boundaries: `../doctrine/layer-boundaries.md`

\- Dignity boundaries: `../compliance/dignity-content-boundaries.md`

\- Chain overview: `../architecture/arcanum-chain.md`
