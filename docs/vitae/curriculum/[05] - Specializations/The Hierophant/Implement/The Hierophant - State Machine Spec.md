---
title: "The Hierophant State Machine Spec"
status: draft
visibility: internal
...
# The Hierophant State Machine Spec
...
**Note:** Interpreting “hydro fan” as **Hierophant** (typo) per the active implementation sequence.

**Transparency Note:** Some earlier uploaded reference PDFs/files have expired in this environment (expected system behavior). This specification is derived from the locked Hierophant canons and current implementation documents and does not depend on expired uploads. If you want alignment against earlier drafts, please re-upload them.

This document defines the **Hierophant specialization state machine**, including states, transitions, guards, failure behavior, and audit requirements. The state machine governs **overlay participation** and must not alter Core Vitae progression.
...
The Hierophant state machine is **overlay-scoped**:
...
* non-claims (no initiation authority, no readiness judgment)  
* risk disclosures (gatekeeping, coercive rites)  
...
**Meaning:** Practitioner may request and complete Hierophant units.
...
* Policy Engine enforces anti-initiation and consent-first pacing invariants
...
### **G2 — No Initiation Authority**

Any SGU content or user interaction implying:

* initiation  
* legitimacy grants  
* priestly authority triggers rejection or stall.

### **G3 — No Readiness Judgment**

No language assessing worthiness, readiness, or qualification is allowed.

### **G4 — No Coercive Threshold Pressure**

No coercive pacing, urgency, or enforced transitions are allowed.
...
### **FM1 — Gatekeeping Drift**

* soften outputs  
* require re-orientation  
* if repeated → STALLED

### **FM2 — Initiation Language**

* block output  
* log violation  
* immediate STALLED

### **FM3 — Coercive Rite Pressure**
...
If the Hierophant state machine enables initiation authority, readiness judgment, coercive transition control, or interference with Core Vitae progression, it is invalid.

This specification defines the **only valid state machine** for the Hierophant specialization.

---

**End of The Hierophant — State Machine Specification**
